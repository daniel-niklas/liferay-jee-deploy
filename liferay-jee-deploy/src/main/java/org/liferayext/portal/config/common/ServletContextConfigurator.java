...
public abstract class ServletContextConfigurator {
	...
	protected <T> Class<? extends T> findClass(String className, Class<T> type) {
		try {
			return Class.forName(className, false, ctx.getClassLoader()).asSubclass(type);
		} catch (ClassCastException e) {
			ctx.log("Klasse " + className + " is not a subclass of " + type.getName());
			return null;
		} catch (ClassNotFoundException e) {
			ctx.log("class " + className + " not found.");
			return null;
		}
	}
	...
	protected ClassLoader findClassLoader(String className) {
		try {
			return Class.forName(className, false, ctx.getClassLoader()).getClassLoader();
		} catch (ClassNotFoundException e) {
			return null;
		}
	}
	...
	protected boolean setInitParameter(String name, String value) {
		if (!ctx.setInitParameter(name, value)) {
			...
			if (!currentValue.equals(ctx.getInitParameter(name))) {
				ctx.log("cannot set init parameter " + name + " -> " + value + " = " + currentValue + "");
				return false;
			}
		}
	...
	protected ServletRegistration findServlet(Class<? extends Servlet> servletClass) {
		for (ServletRegistration servletRegistration : ctx.getServletRegistrations().values()) {
